...
/**
 * Extracts clean 10-digit Indian mobile number from any format.
 * Handles: +91XXXXXXXXXX, 91XXXXXXXXXX, 0XXXXXXXXXX, XXXXXXXXXX
 */
function extractIndianMobile(raw: string): string {
  // Remove all non-digit characters
  const digits = raw.replace(/\D/g, "");

  // Case 1: starts with 91 and total length is 12 → strip country code
...
export default function LeadCard({
  ...
  const mobile10 = extractIndianMobile(lead.phone || "");

  // tel: protocol — +91XXXXXXXXXX (auto-dials with country code)
  const dialPhone = `+91${mobile10}`;

  // WhatsApp URL — 91XXXXXXXXXX (no + sign for wa.me)
  const whatsappPhone = `91${mobile10}`;
    ...
    <>
      <div className="bg-[#0f172a] border border-slate-800/40 rounded-xl p-3 shadow-lg hover:border-blue-500/30 transition-all duration-300 group relative overflow-hidden mb-3">
        ...
        <div className="flex gap-2">
          {/* CALL — uses <a> tag so tel: link works reliably on mobile */}
          <a
            href={`tel:${dialPhone}`}
            onClick={(e) => {
              e.stopPropagation();
              // Open update modal after a short delay so dialer opens first
              setTimeout(() => setModalOpen(true), 300);
            }}
            className="flex-1 h-8 bg-slate-800 hover:bg-slate-700 border border-slate-700/50 rounded-lg flex items-center justify-center gap-1.5 text-slate-300 text-[10px] font-bold transition-all no-underline"
          >
            <Phone size={11} />
            Call
          </a>
